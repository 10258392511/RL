...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        if not self.discrete:
            log_prob = log_prob.sum(dim=1)  # (B, act_dim) -> (B,), summing over all dims in log means multiplying all dims 
        # print(f"inside: log_prob: {log_prob.shape}, obs: {observations.shape}, acts: {actions.shape}\n")
        loss = -log_prob @ advantages # TODO
        ...
        if self.nn_baseline:
            ...
            # print(f"inside: base_line_pred: {baseline_predictions.shape}\n")
