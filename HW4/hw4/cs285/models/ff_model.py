...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        delta_pred_normalized = self.delta_network(concatenated_input)  ## (B, N_obs)
        next_obs_pred = obs_unnormalized + unnormalize(delta_pred_normalized, obs_mean, obs_std)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        data_statistics = {key: ptu.from_numpy(data_statistics[key]) for key in data_statistics}
        prediction = self.forward(obs, acs, **data_statistics)[0]
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        ## assume: observations: (B, N_obs)
        observations, actions, next_observations = ptu.from_numpy(observations), ptu.from_numpy(actions), ptu.from_numpy(next_observations)
        data_statistics = {key: ptu.from_numpy(data_statistics[key]) for key in data_statistics}
        ...
        predictions = self.forward(observations, actions, **data_statistics)[1]
        # criterion = nn.MSELoss()
        loss = self.loss(predictions, target)  # TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
